...
const Nav = () => {
  ...
  const handleSearchItems = async () => {
    try {
      ...
      console.log("handleSearchItems", result.data);
      dispatch(setSearchItems(result.data))
    } catch (error) {
      console.log("handleSearchItems error", error);
    }
  };
  ...
  return (
    <div className="w-full h-[80px] flex items-center justify-between md:justify-center gap-[30px] px-[20px] fixed top-0 z-[9999] bg-[#fff9f6] overflow-visible">
      ...
      <h1 className="text-3xl font-bold mb-2 text-[#ff4d2d]">Eatly</h1>
      {userData.role == "user" && (
      ...
      <div className="flex items-center gap-4">
        ...
        {userData.role == "owner" ? (
          ...
          <>
            {userData.role == "user" && (
              ...
              >
                <PiShoppingCartSimple size={25} className="text-[#ff4d2d] " />
                <span className="absolute right-[-2px] top-[-15px] text-[#ff4d2d]">
                  {cartItems.length}
                </span>
              </div>
            )}
        ...
        {showInfo && (
          <div
            className={`fixed top-[80px] right-[10px] ${userData.role == "deliveryBoy" ? "md:right-[20%] lg:right-[35%]" : "md:right-[10%] lg:right-[25%]"} w-[180px] bg-white shadow-2xl rounded-xl p-[20px] flex flex-col gap-[10px] z-[9999]`}
          >
